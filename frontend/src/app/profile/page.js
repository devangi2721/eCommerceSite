...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const fetchOrderHistory = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/users/orders/history', {
          headers: {
            'Authorization': `Bearer ${token}`
          },
          method: 'GET'
  ...
  const handleCancelOrder = async (orderId) => {
    if (!token) return;
    try {
      const response = await fetch(`http://localhost:5000/api/users/orders/${orderId}/cancel`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
          ...
          {activeTab === 'profile' ? (
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-700 mb-2">Account Statistics</h3>
            ...
            <div className="space-y-6">
              <h3 className="text-lg font-semibold text-gray-700 mb-4">Order History</h3>
